...
tags: ["AI", "Code Quality", "Software Architecture", ".NET", "Developer Productivity", "Claude Code"]
---
...
There's a compounding layer underneath this that doesn't get discussed enough: the model didn't arrive at your codebase neutral. It was trained on the internet, which means it arrived pre-loaded with every anti-pattern the open source world ever published. I wrote about that side of the problem in [Most of the Code AI Learned From Is Garbage](/posts/most-code-ai-learned-from-is-garbage). What happens in production is the intersection: a model biased toward popular-but-wrong patterns, pointed at a codebase full of its own accumulated shortcuts. The amplification works in both directions at once.
...
I've been building production .NET systems this way for the better part of a year, and the teams that navigate this well are the ones who treat CLAUDE.md like a living architecture document — not a one-time setup. Nine months of doing it in the trenches is documented in [Nine Months in the Trenches of Agentic Development](/posts/nine-months-in-the-trenches) — the learning curve, the failures, and what actually works in production.
