...
When we audited our memory files during [the context refactoring described in Part 2](/posts/monolith-to-skills-architecture), we found what we weren't looking for: contradictions. Patterns that described how we *used* to build things, not how we build them now. Rules that conflicted with each other.

The code those rules generated compiled cleanly. Tests passed. CI was quiet. The bugs went to production because the patterns were architecturally correct — just architecturally outdated. Nobody flags that in code review. It's the same invisible failure mode as [Most of the Code AI Learned From Is Garbage](/posts/most-code-ai-learned-from-is-garbage): the problem isn't wrong-looking code, it's plausible-but-wrong code that passes every automated check because the error is architectural, not syntactic.
