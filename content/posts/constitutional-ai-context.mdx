...
date: "2026-03-26"
excerpt: "CLAUDE.md has a governance problem. Without structure, every developer adds rules, nobody removes them, and the AI gets three conflicting answers to the same question. Here's the governance model that fixed it."
...
seriesOrder: 4
image: "/post-images/constitutional-ai-context-og.png"
---

The previous posts in this series are about what happens when AI context goes wrong at the file level: bloat, outdated patterns, skill misfires. This one is about what happens when it goes wrong at the *team* level.

You can have a perfectly structured `CLAUDE.md` that works great for one developer and becomes a governance disaster the moment a second developer starts editing it. The rules that were consistent under one author drift the instant anyone else has write access. Six months in, you have three different answers to "how do we handle soft deletes?" living in three different files, and Claude picks one at random.

That's the constitutional crisis problem. Here's the governance model that solved it.

---

## The Problem With Consensus-Driven Context

Most teams treat their `CLAUDE.md` the way they treat a wiki: everybody can edit it, edits are additive, and nothing gets removed because nobody has the authority to decide what's obsolete.

The result is a rules file that grows by accretion. Each rule added in response to a problem never gets weighed against the rules already there. Eventually you have rules that contradict, rules that duplicate, and rules that describe workflows that no longer exist.

The failure mode for AI context is worse than for wikis because the AI doesn't resolve conflicts by asking for clarification. It picks. Sometimes it picks the most recent rule, sometimes the one with more specificity, sometimes the one that appears first in the file. The developer never knows which rule won. The inconsistency is invisible until a bug surfaces.

The fix isn't better tooling. It's explicit governance structure, the same structure that keeps any law-like system coherent over time.

---

## The Constitutional Hierarchy

The model we use has four tiers, each with defined authority and scope:

```
Tier 1: CLAUDE.md (router)          -- navigation only, ~20 lines
Tier 2: constitution.md (law)       -- immutable principles, versioned
Tier 3: /skills (implementation)    -- procedures, evolving independently
Tier 4: .cursor/rules/*.mdc (hints) -- glob-scoped reminders, thin
```
...
Skills beat cursor rules. Constitution beats skills. This file beats nothing.
...
### Tier 3: Skills (Versioned Implementations)

Skills are where the code lives. They're written by the developer doing the work, reviewed against the constitution, and versioned independently.
...
### Tier 4: Cursor Rules (Thin Hints)
...
Everything else can be added incrementally: the tier system, the amendment process, the skill-to-constitution version references. Add them as the need becomes obvious.
