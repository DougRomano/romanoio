...
Where agents add governance value is in multi-session parallel work. The infrastructure that makes five or six parallel sessions ergonomic is covered in [Claude Remote Agents: Running 10 AI Agents While You're Not at Your Desk](/posts/remote-developer-edge-tmux-tailscale) — the tmux + Tailscale setup that keeps those sessions alive and accessible. The constitutional model here is what keeps them from generating contradictory patterns while they run independently.
