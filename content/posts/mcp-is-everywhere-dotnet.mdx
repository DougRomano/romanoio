...
tags: ["MCP", ".NET", "AI", "Software Architecture", "Agent Framework", "Claude Code", "SpecKit", "Agentic Development"]
---
...
MCP is the USB-C of AI tooling. One protocol, universal connectivity. Build a server once, and Claude Code can use it. Copilot can use it. Your Agent Framework agents can use it. Any future client that speaks the protocol can use it. Before MCP, every framework invented its own tool-calling mechanism. Semantic Kernel had plugins. LangChain had tools. AutoGen had function calls. None of them talked to each other. Anthropic addressed the inefficiency of that multi-step round-tripping at the model level too — [Stop Chatting with Your LLM. Start Compiling.](/posts/stop-chatting-start-compiling) covers Programmatic Tool Calling, which is the complementary shift on the inference side to what MCP handles on the integration side.
...
The .NET blog published *"Build a Model Context Protocol (MCP) Server in C#"* last May. Microsoft Learn has a getting-started page. Azure-Samples has several MCP repositories: a hub repo linking to samples across languages, `remote-mcp-webapp-dotnet` for Azure Web App hosting, `remote-mcp-functions-dotnet` for Azure Functions hosting, and `microsoft/mcp-dotnet-samples` as a broad sample collection. Agent Framework RC has native MCP integration, meaning your agents can discover and call MCP servers without custom plumbing.
...
I want to tell you about something I've been building for the last six to nine months, because it illustrates both how powerful agents already are and how much more powerful they'd be with MCP. The full context for how I got to building production agentic systems is in [Nine Months in the Trenches of Agentic Development](/posts/nine-months-in-the-trenches) — this section describes where that work led.
...
The content gap problem isn't new in the .NET AI ecosystem. I wrote about a closely related one — that most AI training data is low-quality open source and tutorial code — in [Most of the Code AI Learned From Is Garbage](/posts/most-code-ai-learned-from-is-garbage). The implication for MCP is the same: if the content that exists is thin, the models will produce thin MCP implementations. Better community content isn't just good for developers learning; it raises the baseline for what AI agents produce when asked to build MCP servers in C#.
...
Python has hundreds of community MCP servers. TypeScript has dozens. .NET has a handful of official samples and a few blog posts. If that gap doesn't close, enterprise teams evaluating AI stacks will look at the MCP ecosystem, see Python has ten times the tooling, and choose Python. Not because .NET is worse (it's probably better for enterprise AI development) but because the community content isn't there to prove it.
